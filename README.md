...
## Problem Statement
Reticent, a finance startup in its early phase, has successfully completed its product validation period. During this phase, 1,500+ users signed up and transacted with the product. The company collected and stored data from its mobile app and web platform, including user transactions, card information, customer surveys, and app reviews. 

These data were originally stored on a local file server that became inefficient due to performance issues and scalability challenges. As the volume of transactions increased, the company began to face difficulties in managing and retrieving data from the local server.
...
* Transactional Data (OLTP): This includes sensitive user information, transaction records, and card details among others. This data requires real-time processing with minimal latency to support daily operations.
...
Currently, Reticent has already accumulated a significant amount of data from the local server now downloaded and stored as CSV files. As part of this project, the data engineer (Raphael) will ingest this existing CSV data into the appropriate databases.

### Project Goals:
* Transactional Data Storage: A high-performance OLTP system optimized to ensure smooth day-to-day operations.
* Non-Transactional Data Storage: A separate database for read heavy operations, to support querying, user evaluation, and analytics
...
4. Data Ingestion: Correctly and completely ingesting all stored data in CSV files into the respective databases.
...
Considering the thousands and millions of rows in the csv files, manually inserting values into the tables became inefficient. I Used the command line to connect to the database and bulk ingest data into the appropriate tables. The red lines are command output, while, green and yellow are sequence of commands used - interchanging databases 'user_information' to 'surveys'
...
Populated all 7 tables successfully with respective records. Here are 2 tables:
...
* ERD Diagram in PgAdmin (one-to-many relationship)
  
![one-to-many](images/ERD_diagram.png)


# Challenges & Solutions
* When i attempted to ingest the reviews csv file into the app_reviews table, i kept getting an error because Postgres doesn't support non UTF-8 characters. So, i open the file in VScode, used '[^\x00-\x7F]+' to find and replace non UTF-8 characters found in the reviews text column.
...
Using the command line, I successfully ingested large CSV datasets — including over 13 million transaction records — into the appropriate database table. The architecture developed in this project not only supports current operational needs but is also flexible enough to handle future growth in data volume.

Overall, this project strengthened my practical understanding of data modeling, database management, and large scale data handling — key components in the data engineering lifecycle.
